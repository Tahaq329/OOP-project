...
public:
    ...
    bool movePlayer(char direction, bool canFightBoss = true, int monstersDefeated = 0, bool puzzleSolved = false) {
        ...
        if (map[newRow][newCol] == 'B' && !canFightBoss) {
            cout << "A powerful magical barrier blocks your path to the Dark Overlord!\n";
            cout << "Requirements needed:\n";
            if (!puzzleSolved) {
                cout << "- Solve the ancient puzzle\n";
            }
            if (monstersDefeated < 6) {
                cout << "- Defeat " << (6 - monstersDefeated) << " more monsters\n";
            }
            return false;
        }
...
public:
    ...
    void start() {
        ...
        ifstream saveCheck("game_save.txt");
        if (saveCheck.good()) {
            saveCheck.close();
    ...
    void handleCurrentTile() {
        ...
        case 'B':
            if (canFightBoss()) {
                ...
                cout << "The Dark Overlord is sealed behind barriers!\n";
    ...
    void playerTurn() {
        ...
        case 'd': case 'D':
            ...
            if (!gameMap.movePlayer(input, canFightBoss(), monstersDefeated, puzzleSolved)) {

                char direction = input;
                int testRow = gameMap.getPlayerRow();
                int testCol = gameMap.getPlayerCol();

                switch (direction) {
                case 'w': case 'W': testRow--; break;
                case 's': case 'S': testRow++; break;
                case 'a': case 'A': testCol--; break;
                case 'd': case 'D': testCol++; break;
                }

                char targetTile = gameMap.getTileAt(testRow, testCol);
                if (targetTile == '#' || testRow < 0 || testRow >= 10 || testCol < 0 || testCol >= 12) {
                    cout << "Can't move that way! There's a wall blocking your path.\n";
                }
            }
            break;
        default:
